...
- [快速开始](#快速开始)
- [技能详解](#技能详解)
  - [复旦大学 (fudanjobs)](#复旦大学-fudanjobs)
  - [北京大学 (pkujobs)](#北京大学-pkujobs)
  - [清华大学 (qinghuajobs)](#清华大学-qinghuajobs)
  - [浙江大学 (zjujobs)](#浙江大学-zjujobs)
  - [上海交通大学 (sjtujobs)](#上海交通大学-sjtujobs)
- [输出格式](#输出格式)
- [定时任务配置](#定时任务配置)
- [与飞书多维表格集成](#与飞书多维表格集成)
- [常见问题](#常见问题)
...
**数据源**: 复旦大学就业指导与生涯发展处 (career.fudan.edu.cn)
...
**API 特点**:
- 需要 `auth: Basic MTAyNDY6MTAyNDY=` 认证头
...
**数据源**: 北京大学学生就业指导服务中心 (scc.pku.edu.cn)
...
**数据源**: 清华大学学生职业发展指导中心 (career.cic.tsinghua.edu.cn)
...
**API 特点**:
- 无需认证，公开访问
- 列表 API 使用 POST form 方式
- 详情页是服务端渲染的 HTML，需要解析
...
**数据源**: 上海交通大学学生就业服务和职业发展中心 (job.sjtu.edu.cn)
...
| 时间 | 学校 | 脚本 |
|------|------|------|
| 22:00 | 复旦大学 | `fudan_fulltime.py`, `fudan_intern.py`, `fudan_zhaopin.py`, `fudan_talk.py` |
| 22:20 | 北京大学 | `pku_recruit.py`, `pku_intern.py` |
| 22:40 | 清华大学 | `qinghua_recruit.py` |
| 23:00 | 浙江大学 | `zju_recruit.py` |
| 23:20 | 上海交通大学 | `sjtu_recruit.py` |
...
A: 北大就业信息网的后端 API 需要 token 认证，但这个 token 是公开固定值 `111`，无需登录即可获取。

### Q2: 为什么交大每页只能 10 条？

A: 交大就业网对未登录用户有限制，每页最多返回 10 条数据。如需更多数据，需要登录后获取 session。
