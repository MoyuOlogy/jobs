...
北京大学就业信息网 - 日常实习爬虫
==========================================
功能：获取日常实习信息列表及详情（前两页）
API:
  - 列表: POST /f/recruitmentinfo/ajax_frontRecruitinfo (positionType=2)
...
def get_recruit_list(page=1, position_type=2):
    """获取实习列表"""
    data = {
        "pageNo": page,
        "pageSize": PAGE_SIZE,
...
def main():
    ...
    for page in range(1, MAX_PAGES + 1):
        print(f"\n[INFO] 正在获取第 {page} 页...")
        list_result = get_recruit_list(page=page, position_type=2)
    ...
    output = {
        ...
        "数据来源": "北京大学就业信息网 - 日常实习信息",
        "总计数量": len(all_jobs),
        "职位列表": all_jobs,
    }

    output_file = "/root/.openclaw/workspace/output/pku_daily/pku_intern_results.json"
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)

    print(f"\n{'=' * 60}")
    print(f"完成！共获取 {len(all_jobs)} 条日常实习信息")
