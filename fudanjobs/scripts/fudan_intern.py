...
复旦大学就业信息网 - 实习职位爬虫
==========================================
功能：获取实习职位列表及详情，筛选指定日期及以后发布的职位
API:
  - 列表: POST /mobile.php/job/getlist (jobtype=2)
...
def get_intern_list(page=1, size=PAGE_SIZE):
    """获取实习职位列表"""
    data = {
        "jobtype": 2,        # 2=实习
        "isunion": 2,
...
def format_job_summary(item):
    ...
    # 实习特有字段
    work_days = item.get("work_days_name", "")
    practice_month = item.get("practice_month_name", "")
...
def format_job_detail(detail_data):
    ...
    return {
        ...
        "日薪范围": salary,
        "实习天数": job_info.get("work_days_name", ""),
        "实习月数": job_info.get("practice_month_name", ""),
        "转正机会": job_info.get("chance_name", ""),
...
def main():
    ...
    while True:
        print(f"\n[INFO] 正在获取第 {page} 页...")
        result = get_intern_list(page=page)
    ...
    output = {
        ...
        "筛选条件": f"{FILTER_DATE.strftime('%Y-%m-%d')} 及以后发布的实习职位",
        "总计数量": len(all_jobs),
        "职位列表": all_jobs,
    }

    output_file = "/root/.openclaw/workspace/output/fudan_daily/fudan_intern_results.json"
    with open(output_file, "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)

    print(f"\n{'=' * 60}")
    print(f"完成！共获取 {len(all_jobs)} 条实习职位")
